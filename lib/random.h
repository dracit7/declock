#ifndef __DMLOCK_RANDOM_H
#define __DMLOCK_RANDOM_H
...
public:
  ...
  Random() : start(0), end(0), items(999'999'999) {
    set_seed0(0xdeadbeef);
  }

  Random(unsigned long seed, int start, int end) : 
    start(start), end(end), items(end - start) { 
    set_seed0(seed); 
  }
  ...
  // [0.0, 1.0)
  inline double next_uniform_double() {
